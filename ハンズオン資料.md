...
led.on();                              // 点灯
led.off();                             // 消灯
led.pwm({ freq: 1000, duty: 50 });    // 50%の明るさ
```
...
```javascript
led.pwm({ freq: 1000, duty: 100 });  // 最も明るい（100% ON）
led.pwm({ freq: 1000, duty: 50 });   // 中間の明るさ（50% ON）
led.pwm({ freq: 1000, duty: 0 });    // 最も暗い（0% ON = OFF）
```

- `freq`: PWM周波数（Hz）。通常は1000Hz程度
- `duty`: デューティ比（0-100）。ONの時間の割合
...
```javascript
// アナログ入力（A0）からセンサー値を取得
const voltage = obniz.ad0.value;  // 0.0-3.3V
